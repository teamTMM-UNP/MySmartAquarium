...
#define TEMP_INT1 36
#define TEMP_INT2 37
#define LED 26      //RGB LED1
#define LED_ALERT 14 //34 //RGB LED1
#define LED_GREEN 27  //RGB LED1
...
int temp_correct = 23; //Default temp while calc temp_mode
int temp_mode[40];
int count_mode;
...
// --------------------- LIVE DATA ----------------------------
void live_scan(){
  if (digitalRead(LED) || digitalRead(LED_ALERT) || digitalRead(LED_GREEN)) led_active = 0;
    else if (!digitalRead(LED) || !digitalRead(LED_ALERT) || !digitalRead(LED_GREEN)) led_active = 1;
    
    if (digitalRead(LIGHT)) bulb = 0;
    else if (!digitalRead(LIGHT)) bulb = 1;

    if (digitalRead(FEED)) feed = 1;
    else if (!digitalRead(FEED)) feed = 0;

    if (digitalRead(OPT_1)) opt_active = 0;
    else if (!digitalRead(OPT_1)) opt_active = 1;

    if (digitalRead(TERMOS)) term_active = 0;
    else if (!digitalRead(TERMOS)) term_active = 1;
    ...
    // --------------------------- TEMPERATURE SIDE ---------------------------
	  ...
	  if (temp_correct > data_aq.temp_ok+3){
              ...
              if(!is_off)  led_color("COLD");
                ...
                if (offline && !is_off)
                  led_color("OFFLINE");
                else  if(!offline && !is_off)  led_color("NORMAL");
                else led_color("OFF");
	            }
	        }
...
bool testWifi(void) {
  ...
  while ( c < 20 ) {
    ...
    delay(500);
    if (WiFi.status() == 1) x++; //If no SSID available, don't lose time
    if (x >= 5) {
      offline = true;
      return false;
    }
...
void setupAP(void) {
  ...
  do {
  u8g2.setFont(u8g2_font_profont22_tf);
  u8g2.drawStr(20,25,"AP MODE");
  u8g2.drawStr(0,55,"Waiting...");
  }while( u8g2.nextPage() );
...
void led_color(String color){
  ...
  if (color == "AP"){ //purple
    ...
    if (color == "NORMAL"){ //white
    digitalWrite(LED_ALERT,LOW);
    digitalWrite(LED,LOW);
    digitalWrite(LED_GREEN,LOW);
    }
    if (color == "OFFLINE"){ //green
...
void setup()
{
  ...
  u8g2.drawStr(15,30,"My Smart");
  u8g2.drawStr(10,55,"Aquarium");
  }while( u8g2.nextPage() );
  ...
  digitalWrite(TERMOS,HIGH);
  digitalWrite(OPT_1,HIGH);
  led_color("HOT"); // Just for introduction logo and loadings
  delay(1000);
  delay(10);
  if (! rtc.begin()) {
  ...
  led_color("COLD"); // Just for logo introduction and loadings
  delay(1000);
  if (! rtc.isrunning()) {
